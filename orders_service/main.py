...
async def validate_gateway_request(
    ...
    logger.info(f"Orders request from user: {x_user_email} (ID: {x_user_id})")
    return {"user_id": int(x_user_id), "user_email": x_user_email}

# Apply authentication dependency to orders routes
app.include_router(
    orders.router, 
    prefix="/orders", 
    tags=["orders"],
